...
export function canInspectAllParallelBracketParticipants({ sharingMode, visibility }) {
...
export function filterVisibleParallelBracketParticipants({
  participants,
  userId = null,
...
export async function getParallelBracketStatusCounts({ creatorUserId }) {
  const sql = getDb();
  await ensureParallelTournamentSupport(sql);
  const rows = await sql`
    select pt.status, count(*)::integer as count
...
export async function listParallelBrackets({ creatorUserId, status = null, limit = 24, offset = 0 }) {
  ...
  const rows = await sql`
    select
      ...
      aggregate_winner."candidateId" as "winnerEntryId",
      aggregate_winner."candidateName" as "winnerName",
      aggregate_winner.seed as "winnerSeed",
      aggregate_winner."candidateImageUrl" as "winnerImageUrl",
    ...
    left join lateral (
      ...
      from parallel_tournament_participant participant
      join tournament_entry entry on entry.tournament_id = participant.tournament_id
      join candidate on candidate.id = entry.candidate_id
      where participant.parallel_tournament_id = pt.id
        and participant.status = 'complete'
        and entry.final_rank is not null
      group by entry.candidate_id, candidate.name, candidate.image_url
      order by
        "averageRank" asc,
        "rankStdDev" asc,
        seed asc
      limit 1
    ) aggregate_winner on true
    where pt.creator_user_id = ${creatorUserId}
      and pt.archived_at is null
      ${status ? sql`and pt.status = ${status}` : sql``}
    order by
      case pt.status
        when 'active' then 0
        when 'draft' then 1
...
export async function listAccessibleParallelBrackets({
  userId,
  anonymousVoterToken = null,
  statuses = null,
  limit = 24,
  ...
  return sql`
    ...
    where pt.archived_at is null
      ${statuses ? sql`and pt.status in ${sql(statuses)}` : sql``}
      and (
...
export async function listPublicParallelBrackets({
...
export async function getAccessibleParallelBracketById({
  parallelTournamentId,
  userId = null,
  anonymousVoterToken = null
}) {
...
async function getParallelTournamentByIdForCreator({ parallelTournamentId, creatorUserId }) {
  const parallelTournament = await getAccessibleParallelBracketById({
    parallelTournamentId,
...
async function ensureParallelShareLinkSupport(sql) {
  const [row] = await sql`
    ...
    ) as "hasParallelBracketShareLinks"
  `;

  if (!row?.hasParallelBracketShareLinks) {
    throw new Error("PARALLEL_TOURNAMENTS_REQUIRES_MIGRATION");
  }
}

export async function createParallelBracket({
  creatorUserId,
  title,
  description,
  sourcePoolId,
  ...
  return getAccessibleParallelBracketById({
    parallelTournamentId: parallelTournament.id,
    userId: creatorUserId
  });
}

export async function listParallelBracketShareLinks({
  parallelTournamentId,
  creatorUserId
}) {
  const sql = getDb();
...
export async function ensureParallelBracketShareLink({
  parallelTournamentId,
  creatorUserId
}) {
  const sql = getDb();
...
export async function rotateParallelBracketShareLink({
  parallelTournamentId,
  creatorUserId
}) {
  const sql = getDb();
...
export async function getParallelBracketByShareToken({ token, userId }) {
  const sql = getDb();
  await ensureParallelShareLinkSupport(sql);
...
export async function updateParallelBracket({
  parallelTournamentId,
  creatorUserId,
  patch
}) {
  ...
  const nextPlayStyle = Object.hasOwn(patch, "playStyle")
    ? patch.playStyle
    : current.playStyle;
  const nextResultMode = Object.hasOwn(patch, "resultMode")
    ? patch.resultMode
    : current.resultMode;
...
export async function archiveParallelBracket({
  parallelTournamentId,
  creatorUserId
}) {
  const sql = getDb();
...
export async function openParallelBracketParticipant({
  parallelTournamentId,
  userId = null,
  anonymousVoterToken = null
}) {
...
export async function getFeaturedParallelBracketTeaserMatchups({ limit = 6 }) {
  const sql = getDb();
  await ensureParallelTournamentSupport(sql);
...
export async function getParallelBracketAggregateResults({
  parallelTournamentId,
  userId = null,
  anonymousVoterToken = null
}) {
    ...
    completedTournamentIds.length > 0
      ? await sql`
          select
            ...
            candidate.image_url as "candidateImageUrl",
            ${candidateSupport.hasTags ? sql`candidate.tags` : sql`'{}'::text[]`} as "candidateTags",
            avg(entry.final_rank::numeric)::float8 as "averageRank",
            coalesce(stddev_pop(entry.final_rank::numeric), 0)::float8 as "rankStdDev",
          ...
          where entry.tournament_id in ${sql(completedTournamentIds)}
            and entry.final_rank is not null
          group by entry.candidate_id, candidate.name, candidate.description, candidate.image_url, ${candidateSupport.hasTags ? sql`candidate.tags` : sql`'{}'::text[]`}
        `
      : [];
  ...
  const canInspectAllParticipants = canInspectAllParallelBracketParticipants({
    sharingMode: parallelTournament.sharingMode,
    visibility: parallelTournament.visibility
  });
  const visibleParticipants = filterVisibleParallelBracketParticipants({
