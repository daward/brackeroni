import { buildInitialRound, buildNextRound, calculateSwissRoundCount, type BracketEngineEntry, type BracketEngineMatch } from "./rounds";
import {
...
export type TournamentEffortEstimate = {
  candidateCount: number;
  resultMode?: string | null;
  estimatedVotesPerParticipant: number;
  estimatedSynchronizedRounds: number;
  synchronized: boolean;
  confidence: "high" | "medium";
  note: string;
};

type TournamentEffortInput = {
  candidateCount?: number | string | null;
  resultMode?: BracketResultMode | string | null;
  playStyle?: BracketPlayStyle | string;
  advancementMode?: BracketAdvancementMode | string;
};

type SeededRankingSimulationInput = {
  candidateCount: number;
  resultMode: BracketResultMode | string | null | undefined;
  playStyle: BracketPlayStyle | string;
};

type SeededRankingSimulationResult = {
  openMatchCount: number;
  openRoundCount: number;
};

type ResolvedEffortMatch = BracketEngineMatch & {
  countsAsVote: boolean;
};

type NextRankingEntriesInput = {
  entries: BracketEngineEntry[];
  rankedEntryIdSet: Set<string>;
  priorWinnerByPair: Map<string, string>;
};

export function estimateTournamentEffort({
  candidateCount = 0,
  resultMode = "winner_only",
  playStyle = "fixed_bracket",
  advancementMode = "vote_winner"
}: TournamentEffortInput = {}): TournamentEffortEstimate {
  ...
  if (isParallelResultMode(resultMode)) {
    const participantEstimate: TournamentEffortEstimate = estimateTournamentEffort({
      candidateCount: normalizedCandidateCount,
...
function simulateSeededRankingEffort({ candidateCount, resultMode, playStyle }: SeededRankingSimulationInput): SeededRankingSimulationResult {
  const entries: BracketEngineEntry[] = Array.from({ length: candidateCount }, (_, index) => ({
    id: `entry-${index + 1}`,
    seed: index + 1,
  }));
  const rankedEntryIds: string[] = [];
  const rankedEntryIdSet = new Set<string>();
  const priorWinnerByPair = new Map<string, string>();
  ...
  while (rankedEntryIds.length < targetRankCount) {
    ...
    const advancingEntries = resolvedMatches
      .map((match) => entries.find((entry) => entry.id === match.winnerEntryId))
      .filter((entry): entry is BracketEngineEntry => Boolean(entry));
  ...
  function rankEntry(entry: BracketEngineEntry) {
...
function resolveKnownOrSeededWinners(matches: BracketEngineMatch[], priorWinnerByPair: Map<string, string>): ResolvedEffortMatch[] {
...
function getNextRankingEntries({ entries, rankedEntryIdSet, priorWinnerByPair }: NextRankingEntriesInput): BracketEngineEntry[] {
...
function getPairKey(leftEntryId: string, rightEntryId: string): string {
