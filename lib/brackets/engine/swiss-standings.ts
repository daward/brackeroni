...
export type SwissStanding = SwissEntry & {
  wins: number;
  losses: number;
  played: number;
  opponents: string[];
...
export function buildSwissStandings(entries: SwissEntry[], matches: SwissMatch[]): SwissStanding[] {
  const statsById = new Map<string, Omit<SwissStanding, "buchholz">>(
    entries.map((entry) => [
      entry.id,
      {
        id: entry.id,
  ...
  for (const match of matches) {
    ...
    const winner = byeWinnerId ? statsById.get(byeWinnerId) : null;
