export type BracketEnginePlayStyle = "fixed_bracket" | "reseed" | string;

export type BracketEngineEntry = {
  id: string;
  seed: number;
  subSeed?: number | null;
  score?: number | null;
...
export type BracketEngineMatch = {
  leftEntryId: string | null;
  rightEntryId: string | null;
  leftSeed: number | null;
  rightSeed: number | null;
  leftSlotType: "entry";
  rightSlotType: "entry" | "bye";
  status: "open" | "auto_resolved";
  resolutionSource: "bye" | null;
  winnerEntryId: string | null;
  pairKey: string;
  countsAsVote?: boolean;
  leftVoteCount?: number | null;
  rightVoteCount?: number | null;
};

type SeedingSubBracketInput = {
  id?: string | null;
  index?: number | null;
  name?: string | null;
};

export type BracketEngineSeedingStructure = {
  subBrackets?: SeedingSubBracketInput[] | null;
  entryBrackets?: Record<string, string> | null;
};

type NormalizedSeedingStructure = {
  subBrackets: Array<{ id: string; index: number; name: string }>;
  entryBrackets: Record<string, string>;
};

type BracketGroup = {
  id: string;
  entries: BracketEngineEntry[];
};

type BuildNextRoundOptions = {
  playStyle: BracketEnginePlayStyle;
  roundNumber: number;
  seedingStructure?: BracketEngineSeedingStructure;
};

type BuildSwissRoundOptions = {
  roundNumber: number;
  priorMatches?: BracketEngineMatch[];
};

type SwissEntry = BracketEngineEntry & {
  score: number;
  buchholz: number;
};
...
export function calculateSwissRoundCount(entryCount: number): number {
  if (entryCount <= 1) {
    return 0;
  }
...
function normalizeSeedingStructure(seedingStructure: BracketEngineSeedingStructure = {}): NormalizedSeedingStructure {
  ...
  const subBrackets = [...rawSubBrackets]
    .filter((subBracket): subBracket is SeedingSubBracketInput & { id: string } => (
      typeof subBracket?.id === "string" && subBracket.id.length > 0 && subBracket.id !== "__root__"
    ))
    .sort((left, right) => {
...
function hasSubBracketAssignments(entries: BracketEngineEntry[], seedingStructure: BracketEngineSeedingStructure = {}): boolean {
  const normalized = normalizeSeedingStructure(seedingStructure);
  return entries.some((entry) => normalized.entryBrackets[entry.id]);
}

function localizeGroupEntries(entries: BracketEngineEntry[]): BracketEngineEntry[] {
...
function buildBracketGroups(entries: BracketEngineEntry[], seedingStructure: BracketEngineSeedingStructure = {}): BracketGroup[] {
  const normalized = normalizeSeedingStructure(seedingStructure);
  const orderedGroups = normalized.subBrackets.map((subBracket) => ({
    id: subBracket.id,
    entries: [] as BracketEngineEntry[]
  }));
  const rootGroup = {
    id: "__root__",
    entries: [] as BracketEngineEntry[]
...
function prefixMatches(matches: BracketEngineMatch[], prefix: string): BracketEngineMatch[] {
...
function buildGroupByeMatch(entry: BracketEngineEntry, roundNumber: number, groupId: string): BracketEngineMatch {
  return {
...
function buildSubBracketRound(
  entries: BracketEngineEntry[],
  roundNumber: number,
  playStyle: BracketEnginePlayStyle = "reseed",
  seedingStructure: BracketEngineSeedingStructure = {}
): BracketEngineMatch[] {
  ...
  const matches: BracketEngineMatch[] = [];
...
function buildSeededRound(entries: BracketEngineEntry[], roundNumber: number): BracketEngineMatch[] {
  ...
  const duplicateSeedEntries = new Map<number, BracketEngineEntry[]>();
...
function buildFixedBracketRound(entries: BracketEngineEntry[], roundNumber: number): BracketEngineMatch[] {
...
export function buildInitialRound(
  entries: BracketEngineEntry[],
  seedingStructure: BracketEngineSeedingStructure = {}
): BracketEngineMatch[] {
...
export function buildNextRound(entries: BracketEngineEntry[], { playStyle, roundNumber, seedingStructure = {} }: BuildNextRoundOptions): BracketEngineMatch[] {
  if (hasSubBracketAssignments(entries, seedingStructure)) {
...
export function buildSwissRound(entries: BracketEngineEntry[], { roundNumber, priorMatches = [] }: BuildSwissRoundOptions): BracketEngineMatch[] {
  const normalizedEntries: SwissEntry[] = [...entries].map((entry) => ({
    ...entry,
...
function buildSwissFirstRoundOrder(entries: SwissEntry[]): SwissEntry[] {
  ...
  const orderedEntries: SwissEntry[] = [];
...
function compareSwissEntries(left: SwissEntry, right: SwissEntry): number {
...
function buildSwissPairKey(leftEntryId: string, rightEntryId: string): string {
  return [leftEntryId, rightEntryId].sort().join(":");
}

function findSwissByeIndex(entries: SwissEntry[], byeRecipients: Set<string>): number {
...
function pairSwissEntries(entries: SwissEntry[], rematchSet: Set<string>): Array<[SwissEntry, SwissEntry]> | null {
  if (entries.length === 0) {
    return [];
  }
