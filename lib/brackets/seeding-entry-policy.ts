...
import { buildSeedingGroups, createSeedingStructure, normalizeSeedingStructure } from "@/lib/brackets/seeding-draft";
...
export function normalizeSeedingEntries(entries: SeedingEntryRecord[]): SeedingEntry[] {
  return [...entries].map((entry) => ({ ...entry, subSeed: Number(entry.subSeed || 0), isEmptySlot: Boolean(entry.isEmptySlot) }));
}

export function hydrateSeedingEntries(entries: SeedingEntryRecord[] | null | undefined): SeedingEntry[] {
