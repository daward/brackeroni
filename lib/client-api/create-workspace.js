...
export function listPools({ limit = 24, offset = 0 } = {}) {
  const params = new URLSearchParams();
  if (limit != null) params.set("limit", String(limit));
  if (offset != null) params.set("offset", String(offset));
  const query = params.toString();

  return requestJson(`/api/pools${query ? `?${query}` : ""}`, {
    cache: "no-store",
...
export function getPool(poolId, { candidateLimit = null, candidateOffset = 0 } = {}) {
  const params = new URLSearchParams();
  if (candidateLimit) params.set("candidateLimit", String(candidateLimit));
  if (candidateOffset) params.set("candidateOffset", String(candidateOffset));
  return requestJson(`/api/pools/${poolId}${params.size ? `?${params}` : ""}`, {
    cache: "no-store",
...
export function generateCandidatesInPool(poolId, payload) {
  return requestJson(`/api/pools/${poolId}/generations`, {
    method: "POST",
    body: payload,
    errorMessage: "Failed to generate candidates."
  });
}

export function listTournaments({ limit = 24, offset = 0, status = null } = {}) {
  const params = new URLSearchParams({ limit: String(limit), offset: String(offset) });
  if (status) params.set("status", status);
  return requestJson(`/api/brackets?${params}`, {
    cache: "no-store",
...
export function createTournament(payload) {
  return requestJson("/api/brackets", {
    method: "POST",
    body: payload,
...
export function previewBracketIntent(prompt) {
  return requestJson("/api/bracket-intents", {
    method: "POST",
    body: { prompt, action: "preview" },
    errorMessage: "Failed to preview bracket prompt."
  });
}

export function createBracketFromIntent(prompt) {
  return requestJson("/api/bracket-intents", {
    method: "POST",
    body: { prompt, action: "create" },
    errorMessage: "Failed to create bracket from prompt."
  });
}
...
export function openNextTournamentRound(tournamentId) {
  return updateTournament(tournamentId, { openNextRound: true });
}
...
export function rerunTournament(tournamentId) {
  return requestJson(`/api/brackets/${tournamentId}/rerun-drafts`, {
    method: "POST",
...
export function listTournamentInvites(tournamentId) {
  return requestJson(`/api/brackets/${tournamentId}/invites`, {
    cache: "no-store",
...
export function listTournamentShareLinks(tournamentId) {
  return requestJson(`/api/brackets/${tournamentId}/links`, {
    cache: "no-store",
...
export function listTournamentEntries(tournamentId) {
  return requestJson(`/api/brackets/${tournamentId}/entries`, {
    cache: "no-store",
...
export function listParallelTournaments({ limit = 24, offset = 0, status = null } = {}) {
  const params = new URLSearchParams({ limit: String(limit), offset: String(offset) });
  if (status) params.set("status", status);
  return requestJson(`/api/parallel-brackets?${params}`, {
    cache: "no-store",
...
export function getParallelTournament(parallelTournamentId) {
  return requestJson(`/api/parallel-brackets/${parallelTournamentId}`, {
    cache: "no-store",
...
export function createParallelTournament(payload) {
  return requestJson("/api/parallel-brackets", {
    method: "POST",
    body: payload,
...
export function listParallelTournamentShareLinks(parallelTournamentId) {
  return requestJson(`/api/parallel-brackets/${parallelTournamentId}/links`, {
    cache: "no-store",
    errorMessage: "Failed to load share links."
  });
}
