...
function parseRankedHeadingCandidates(html, pageUrl) {
  const normalizedHtml = extractFragmentSectionHtml(html, pageUrl);
  ...
  const headingMatches = [
    ...normalizedHtml.matchAll(
      /<h([1-6])\b[^>]*>([\s\S]*?)(?:<strong>\s*)?(\d+)\.\s*([^<]+?)(?:\s*<\/strong>)?[\s\S]*?<\/h\1>/gi
    )
  ];
  ...
  for (let index = 0; index < headingMatches.length; index += 1) {
    ...
    const end = headingMatches[index + 1]?.index ?? normalizedHtml.length;
    const block = normalizedHtml.slice(start, end);
    const rank = Number(current[3]);
    const label = normalizeLine(collapseWhitespace(stripHtmlTags(current[4])));
    ...
    const description =
      paragraphMatches
        .map((match) => normalizeLine(collapseWhitespace(stripHtmlTags(match[1]))))
        .find((text) => text && text !== label && !/^\d+\.\s*/.test(text)) || null;

    const imageTagMatch =
      block.match(
        /<img\b[^>]*(?:data-image=|data-src=|srcset=|src=)["'][^"']+["'][^>]*>/i
      ) || block.match(/<img\b[^>]*>/i);
    const imageUrl = imageTagMatch ? extractBestImageUrlFromTag(imageTagMatch[0], pageUrl) : null;
    ...
    candidates.push({
      ...
      description: description && !isStatOnlyDescription(description) ? description : null,
      rationale: null,
      score: Number.isFinite(rank) && rank > 0 ? Math.max(0, 1 - (rank - 1) / 100) : 1,
      imageUrl,
  ...
  const deduped = dedupeCandidates(candidates);
  return deduped.length >= 10 ? deduped : [];
}
...
export async function extractCandidatesWithGeminiForPools({
  ...
  const captureWarnings = analyzeCapturedHtml({ html, pageUrl });
  ...
  if (html) {
    ...
    const rankedHeadingCandidates = parseRankedHeadingCandidates(html, pageUrl);

    if (rankedHeadingCandidates.length >= 10) {
      return {
        candidates: enrichCandidatesFromHtml(rankedHeadingCandidates, html, pageUrl),
        citations: [],
        model: "ranked-heading-parser",
        warnings: captureWarnings
      };
    }
  ...
  return {
    ...
    warnings: [...captureWarnings, ...(result.warnings || [])]
  };
}
