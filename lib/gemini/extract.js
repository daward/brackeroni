    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          label: {
            type: "string",
            description: "The candidate's display label."
          },
          rationale: {
            anyOf: [{ type: "string" }, { type: "null" }],
            description: "Why this candidate was selected from the source material."
          },
          score: {
            anyOf: [{ type: "number" }, { type: "null" }],
            description: "Confidence or relevance score between 0 and 1 when possible."
          },
          sourceUrl: {
            anyOf: [{ type: "string" }, { type: "null" }],
            description: "Best source URL for the candidate when known."
          },
          sourceTitle: {
            anyOf: [{ type: "string" }, { type: "null" }],
            description: "Best source title for the candidate when known."
          },
          excerpt: {
            anyOf: [{ type: "string" }, { type: "null" }],
            description: "Short supporting excerpt or snippet."
          }
        },
...
function splitTextIntoChunks(text) {
  ...
  for (let index = 0; index < lines.length; index += CHUNK_LINE_LIMIT) {
    const block = lines.slice(index, index + CHUNK_LINE_LIMIT);
...
function normalizeCandidates(value) {
  ...
  return value.candidates
    .map((candidate) => ({
      label: normalizeLine(String(candidate.label || "").trim()),
      rationale: candidate.rationale ? String(candidate.rationale).trim() : null,
      score:
      ...
      excerpt: candidate.excerpt ? String(candidate.excerpt).trim() : null
    }))
...
export async function extractCandidatesWithGemini({ prompt, text = null, html = null, urls = [], model }) {
