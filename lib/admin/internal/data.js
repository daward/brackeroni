...
export async function deleteArchivedPool({ poolId }) {
  ...
  const [pool] = parallelSupport.hasParallelTournamentTable
    ? await sql`
        ...
        where id = ${poolId}
          ...
          and not exists (
            ...
            where pt.source_pool_id = candidate_pool.id
          )
        returning id
      `
    : await sql`
...
export async function deleteAllArchivedMaterial() {
  ...
  return sql.begin(async (tx) => {
    let deletedArchivedParallelTournaments = [];

    if (parallelSupport.hasParallelTournamentTable) {
      const childTournamentIds = await tx`
        select participant.tournament_id as id
        from parallel_tournament_participant participant
        join parallel_tournament parent
          on parent.id = participant.parallel_tournament_id
      ...
      await tx`
        delete from parallel_tournament_participant
        where parallel_tournament_id in (
          select id
          from parallel_tournament
      ...
      deletedArchivedParallelTournaments = await tx`
        delete from parallel_tournament
        where archived_at is not null
        returning id
      `;
    ...
    const deletedArchivedPools = parallelSupport.hasParallelTournamentTable
      ? await tx`
          delete from candidate_pool
          where archived_at is not null
            and not exists (
    ...
    return {
      ...
      deletedParallelTournamentCount: deletedArchivedParallelTournaments.length,
