import { z } from "zod";
import { BRACKET_INTENT_PRESET_IDS } from "@/lib/brackets/intent-presets";

const intentPresetSchema = z.enum(BRACKET_INTENT_PRESET_IDS).optional().nullable();
...
export const tournamentCreateSchema = z.object({
  ...
  resultMode: z.enum(["winner_only", "full_ranking", "partial_ranking", "fast_full_rank"]),
  tieBreakMode: z.enum(["higher_seed_wins", "random"]),
  seedCandidateIds: z.array(z.string().uuid()).min(2).optional(),
  intentPreset: intentPresetSchema,
  advancementMode: z
    .enum(["vote_winner", "manual_winner"])
    .optional()
    .default("vote_winner")
});
...
export const tournamentUpdateSchema = z
  .object({
    ...
    resultMode: z.enum(["winner_only", "full_ranking", "partial_ranking", "fast_full_rank"]).optional(),
    tieBreakMode: z.enum(["higher_seed_wins", "random"]).optional(),
    intentPreset: intentPresetSchema,
    advancementMode: z.enum(["vote_winner", "manual_winner"]).optional(),
    status: z.enum(["draft", "active", "complete"]).optional(),
    closeCurrentRound: z.boolean().optional(),
    openNextRound: z.boolean().optional(),
