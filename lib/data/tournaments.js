...
import { closeCurrentRound, openNextPublicRound } from "@/lib/data/rounds";
...
export async function updateTournament({ tournamentId, creatorUserId, patch }) {
  ...
  const shouldOpenNextRound = patch.openNextRound === true;
  ...
  if (shouldCloseCurrentRound || shouldOpenNextRound) {
    await sql.begin(async (tx) => {
      ...
      if (shouldOpenNextRound) {
        await openNextPublicRound({
          tx,
          tournamentId,
          creatorUserId
        });
