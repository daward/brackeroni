...
export async function getFeaturedPublicMatchup({ userId = null, anonymousVoterToken = null }) {
  const sql = getDb();
  const typedUserId = userId ?? null;
  const typedAnonymousVoterToken = anonymousVoterToken ?? null;

  const [featuredRound] = await sql`
    select
      ...
      count(*) filter (where user_vote.id is null)::integer as "availableUnvotedMatchCount",
      count(*)::integer as "openMatchCount"
    from tournament t
    ...
    where t.archived_at is null
      and t.visibility = 'public_listed'
      and t.status = 'active'
      and (${typedUserId}::uuid is not null or t.voting_access = 'anyone')
    group by
      t.id,
      t.title,
      t.description,
      ...
      r.sequence_number
    order by
      count(*) filter (where user_vote.id is null) desc,
      coalesce(t.last_vote_at, t.updated_at) desc,
  ...
  const matches = await sql`
    ...
    where m.round_id = ${featuredRound.roundId}
      and m.status = 'open'
    order by
...
export async function createTournament({
  ...
  const roundClosureMode =
    sharingMode === "private" ? "automatic_when_settled" : "all_votes_received";
  ...
  const rerunTournament = await sql.begin(async (tx) => {
    const [createdTournament] = await tx`
      ...
      values (
        ...
        ${sourceTournament.visibility},
        ${sourceTournament.votingAccess},
...
export async function updateTournament({ tournamentId, creatorUserId, patch }) {
  ...
  const nextRoundClosureMode =
    nextSharingMode === "private" ? "automatic_when_settled" : "all_votes_received";
