...
import { getParticipantChildResultMode } from "@/lib/bracket-modes";
...
export async function getParallelTournamentStatusCounts({ creatorUserId }) {
  const sql = getDb();
...
export async function listParallelTournaments({ creatorUserId, status = null, limit = 24, offset = 0 }) {
  const sql = getDb();
