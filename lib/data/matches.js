...
export async function listMatchesForTournament({ tournamentId, creatorUserId, userId }) {
  const sql = getDb();

  const tournament = await getTournamentAccess({ tournamentId, userId, creatorUserId });
  ...
  const matches = await sql`
    ...
    order by r.sequence_number asc, m.created_at asc
  `;
...
export async function submitVote({ matchId, userId, selectedEntryId }) {
  const sql = getDb();
  ...
  return sql.begin(async (tx) => {
    ...
    await tx`
      insert into vote (match_id, user_id, selected_entry_id)
      values (${matchId}, ${userId}, ${selectedEntryId})
    `;

    if (
      match.tournamentStatus === "active" &&
      match.roundClosureMode === "automatic_when_settled"
    ) {
...
export async function getTournamentAccess({ tournamentId, userId, creatorUserId }) {
  const sql = getDb();
...
async function assertVotingAccess({ sql, tournamentId, sharingMode, creatorUserId, userId }) {
