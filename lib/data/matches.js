...
export async function listMatchesForTournament({
  ...
  const matches = await sql`
    ...
    left join vote user_vote
     ...
     and (
       (${userId} is not null and user_vote.user_id = ${userId})
       or (
         ${anonymousVoterToken ?? null} is not null
         and user_vote.anonymous_voter_token = ${anonymousVoterToken ?? null}
       )
     )
...
export async function submitVote({
  ...
  return sql.begin(async (tx) => {
    ...
    const existing = await tx`
      ...
      where match_id = ${matchId}
        and (
          ...
          or (
            ${anonymousVoterToken} is not null
            and anonymous_voter_token = ${anonymousVoterToken}
          )
        )
      limit 1
    `;
    ...
    await tx`
      ...
      values (${matchId}, ${userId}, ${anonymousVoterToken}, ${selectedEntryId})
    `;
