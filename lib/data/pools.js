...
export async function createPool({ creatorUserId, name, description, candidates = [] }) {
  const sql = getDb();
  ...
  return getPoolById({ poolId: pool.id, creatorUserId });
}
...
export async function removeCandidateFromPool({ poolId, creatorUserId, candidateId }) {
  const sql = getDb();
  await getPoolById({ poolId, creatorUserId });
...
export async function mergePoolIntoPool({ poolId, sourcePoolId, creatorUserId }) {
  ...
  const sql = getDb();
  await getPoolById({ poolId, creatorUserId });
  await getPoolById({ poolId: sourcePoolId, creatorUserId });
