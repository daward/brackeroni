...
import { recordJsonResponse, withLocalDbTrace } from "@/lib/diagnostics/local-db-trace";

export function json(data, init) {
  recordJsonResponse(data);
...
export function withRouteErrorHandling(handler) {
  return async function wrappedHandler(...args) {
    ...
    return withLocalDbTrace(request, async () => {
      try {
      ...
      if (error.message === "PARALLEL_TOURNAMENTS_REQUIRES_MIGRATION") {
        return errorResponse(
          400,
          "PARALLEL_TOURNAMENTS_REQUIRES_MIGRATION",
          "Run the parallel-tournaments migration before using parallel brackets."
        );
      }

      if (error.message === "PARALLEL_TOURNAMENT_CLOSED") {
        return errorResponse(
          400,
          "PARALLEL_TOURNAMENT_CLOSED",
          "This parallel bracket has already been closed."
        );
      }

      if (error.message === "POOL_FEATURED_REQUIRES_MIGRATION") {
        return errorResponse(
          400,
          "POOL_FEATURED_REQUIRES_MIGRATION",
          "Run the featured-pools migration before featuring pools on the home page."
        );
      }
      ...
      if (error.message === "BRACKET_INTENT_AMBIGUOUS_POOL") {
        return errorResponse(
          409,
          "BRACKET_INTENT_AMBIGUOUS_POOL",
          "That prompt matched more than one pool. Name the pool more exactly."
        );
      }

      if (error.message === "BRACKET_INTENT_NOT_ENOUGH_CANDIDATES") {
        return errorResponse(
          400,
          "BRACKET_INTENT_NOT_ENOUGH_CANDIDATES",
          "The prompt did not produce enough candidates for a bracket."
        );
      }
      ...
      if (error.message === "MATCH_WINNER_NOT_EDITABLE") {
        return errorResponse(
          400,
          "MATCH_WINNER_NOT_EDITABLE",
          "That match winner cannot be changed right now."
        );
      }
      ...
      if (error.message === "ROUND_RESULTS_INCOMPLETE") {
        return errorResponse(
          409,
          "ROUND_RESULTS_INCOMPLETE",
          "Pick a winner for every open match before closing the round."
        );
      }
      ...
      if (error.message === "ROUND_NOT_REVEALABLE") {
        return errorResponse(
          400,
          "ROUND_NOT_REVEALABLE",
          "Only a closed round can be revealed by the bracket creator."
        );
      }
