...
import { listAccessibleTournaments } from "@/lib/data/tournaments";
...
export default async function VotePage({ searchParams }) {
  await requireCurrentUserPage("/vote");
  const params = (await searchParams) ?? {};
  const user = await getCurrentUser();
  const tournaments = await listAccessibleTournaments({ userId: user.id });
