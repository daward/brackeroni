...
import { parallelBracket } from "@/lib/brackets";
...
export const GET = withRouteErrorHandling(async function GET(request, { params }) {
  ...
  const items = await parallelBracket({ parallelBracketId: parallelTournamentId, creatorUserId: user.id }).listShareLinks();
...
export const POST = withRouteErrorHandling(async function POST(request, { params }) {
  ...
  const currentBracket = parallelBracket({ parallelBracketId: parallelTournamentId, creatorUserId: user.id });
