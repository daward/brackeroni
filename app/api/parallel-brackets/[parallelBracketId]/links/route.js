...
export const GET = withRouteErrorHandling(async function GET(request, { params }) {
  ...
  const items = await parallelBracket({ parallelBracketId, creatorUserId: user.id }).listShareLinks();
...
export const POST = withRouteErrorHandling(async function POST(request, { params }) {
  ...
  const currentBracket = parallelBracket({ parallelBracketId, creatorUserId: user.id });
