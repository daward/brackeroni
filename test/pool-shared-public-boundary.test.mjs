...
const sharedDirectory = path.join(componentDirectory, "pools", "shared");
...
test("pool shared internals are not imported outside the shared feature", () => {
  const violations = getSourceFiles(componentDirectory)
    ...
    .filter((filePath) => fs.readFileSync(filePath, "utf8").includes("components/pools/shared/internal"));
