...
const internalImportMarker = ["components", "brackets", "configuration", "internal"].join("/");
const configurationDirectory = path.join(rootDirectory, "components", "brackets", "configuration");
...
test("bracket configuration internals are not imported outside the feature", () => {
  const violations = sourceDirectories
    .flatMap(getSourceFiles)
    .filter((filePath) => filePath !== boundaryTestPath)
    .filter((filePath) => !filePath.startsWith(configurationDirectory))
    .filter((filePath) => fs.readFileSync(filePath, "utf8").includes(internalImportMarker));

  assert.deepEqual(violations, []);
});

test("bracket configuration keeps only its public API at the feature root", () => {
