...
type UseTournamentSharingActionsProps = {
  tournaments: WorkspaceTournament[];
  tournamentShareLinks: TournamentShareLinksState;
  setTournamentShareLink: (tournamentId: string, shareLink: WorkspaceShareLink) => void;
...
export function useTournamentSharingActions({
  ...
  setErrorMessage,
  setSuccessMessage,
}: UseTournamentSharingActionsProps) {
  async function handleEnsureShareLink(tournamentId: string, { rotate = false, silent = false }: { rotate?: boolean; silent?: boolean } = {}) {
    ...
    try {
      const data = isParallelParent ? await ensureParallelTournamentShareLink(tournamentId, { rotate }) : await ensureTournamentShareLink(tournamentId, { rotate });
      ...
      setErrorMessage(getErrorMessage(error, "Failed to prepare share link."));
      return null;
    } finally {
      endAction(actionKey);
    }
  ...
  async function handleCopyShareLink(tournamentId: string) {
    setErrorMessage("");
    setSuccessMessage("");
