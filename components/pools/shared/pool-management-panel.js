...
function isGeneratedImportDescription(description, sourceUrl) {
  if (!description || !sourceUrl) return false;
...
export function PoolManagementPanel({
  ...
  const shouldShowDescription = !isGeneratedImportDescription(
    draft?.description ?? pool?.description,
    pool?.importSourceUrl
  );

  return (
    <section className={`pool-management-panel ${showPanelRule ? "" : "pool-management-panel-no-rule"} ${className}`}>
      {pool ? (
        <div className="flex items-start justify-between gap-6">
          <div className="min-w-0 flex-1">
            ...
            {compactDetails && shouldShowDescription ? (
              <div className={`mt-3 flex flex-wrap items-start justify-between gap-x-6 gap-y-2 ${showDetailsRule ? "border-t border-[var(--line)] pt-3" : ""}`}>
                <div className="min-w-0 flex-1">
