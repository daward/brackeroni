...
export function usePoolCandidateActions({
  ...
  useEffect(() => {
    const timers = Object.entries(candidateDrafts).map(([poolId, draft]) => {
      ...
      if (candidateName.length < 2 || imageSuggestionLoading[poolId] || imageSuggestionQuery[poolId] === candidateName) {
        return null;
      }
  ...
  async function syncLinkedDraftBrackets(poolId, patch) {
    const linkedDraftBrackets = tournaments.filter((tournament) => tournament.status === "draft" && tournament.sourcePoolId === poolId);
  ...
  async function handleRemoveCandidateFromPool(poolId, candidate) {
    ...
    setErrorMessage("");
    setSuccessMessage("");
    const scrollTopBeforeRemoval = typeof window !== "undefined" ? window.scrollY || window.pageYOffset || document.documentElement.scrollTop || 0 : 0;
    ...
    try {
      for (const candidate of missingImageCandidates) {
        try {
          ...
          const bestSuggestion = (data.items || []).find((item) => isStrongSuggestedImageMatch(candidate.name, item));
      ...
      setSuccessMessage(`Filled ${appliedCount} missing image${appliedCount === 1 ? "" : "s"}. ` + `${skippedCount} skipped.${failedCount > 0 ? ` ${failedCount} failed.` : ""}`);
