...
type Props = {
  ...
  interaction: { readOnly: boolean; activeTagFilter: string; expandedCandidateId: string | null; removingCandidateId: string | null; onCandidateActivate: (candidate: PoolCandidate) => void; onRemoveCandidate: (candidate: PoolCandidate) => void };
  children: ReactNode;
};
        ...
        {candidates.length === 0 && (readOnly || activeTagFilter) ? (
          <div className={styles.emptyState}>
            <span className="ui-copy">
              {activeTagFilter ? `No candidates match the "${activeTagFilter}" tag.` : collection.emptyMessage}
            </span>
          </div>
        ) : (
