...
type TagState = {
  ...
  onClose: () => void;
  onActiveFilterChange: (tag: string) => void;
  onLowValueThresholdChange: (threshold: string) => void;
};

type Props = {
  tags: TagState;
  management: Omit<CandidateTagManagement, "showControl"> & { readOnly: boolean };
};

export function CandidateTagDrawer({ tags, management }: Props) {
  const { isOpen, sortedTags, activeTagFilter, lowValueThreshold, onClose, onActiveFilterChange, onLowValueThresholdChange } = tags;
  ...
  return (
    <SideDrawer size="narrow" title="Pool Tags" description="Filter the pool or remove a tag everywhere." onClose={onClose}>
      <SideDrawerBody>
        {sortedTags.length === 0 ? (
