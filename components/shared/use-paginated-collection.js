 ...
 */
export function usePaginatedCollection({ resourceKey, initialItems, initialPagination, loadPage, getId = getItemId }) {
  ...
  const initialIdsRef = useRef(new Set(initialItems.map(getId).filter(Boolean)));
  const nextOffsetRef = useRef(initialItems.length);
  const loadingRef = useRef(false);

  useEffect(() => {
    const nextInitialIds = new Set(initialItems.map(getId).filter(Boolean));
