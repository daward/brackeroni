...
export function SideDrawer({ size = "wide", title, description = "", onClose, children }: SideDrawerProps) {
  return (
    <div className={styles.overlay}>
      ...
      <aside className={`${styles.drawer} ${size === "narrow" ? styles.drawerNarrow : styles.drawerWide}`} aria-modal="true" role="dialog" aria-label={title}>
        <header className={styles.header}>
          ...
          <button type="button" onClick={onClose} className="ui-button ui-button-muted">Close</button>
        </header>
        {children}
      </aside>
    </div>
